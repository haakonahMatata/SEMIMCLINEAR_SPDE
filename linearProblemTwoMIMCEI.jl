# Code for generating figure 4 in manuscript
# CostRatio for MLMC methods in runs will be 
...
function main()
    ...
    runalltests(linear_multNoiseProblemShifted(ν=10//9))
...
function mlmctest(pData,rngVec)
    ...
    costVecMLMC = zeros(length(tol)-1);
    errorVecMLMC = zeros(length(tol)-1);
    costVecMIMC = zeros(length(tol));
    errorVecMIMC = zeros(length(tol));
    costVecMIMC2 = zeros(length(tol));
    errorVecMIMC2 = zeros(length(tol));
    ...
    for i in 1:length(tol)
        ...
        estU, cost       = mimcestimator(pData,rngVec, tol[i], reducedSample=true);
        costVecMIMC2[i]  = cost;
        errorVecMIMC2[i] = sum( singleDifference(pseudoRefSol,estU).^2 );
    ...
    scatterlines!(ax2,tol, costVecMIMC ,linewidth=2, color=:black, markersize=14, label=L"MIMC1")
    scatterlines!(ax2,tol, costVecMIMC2 ,linewidth=2, marker =:cross, markersize=14, color=:black, label=L"MIMC2")
    lines!(ax2,tol[1:length(errorVecMLMC)], slopeLineMLMC ,linewidth=2, color=:black, linestyle=:dash, label=slopeTextMLMC)
    lines!(ax2,tol, slopeLineMIMC,linewidth=2, color=:black, linestyle=:dashdotdot, label=slopeTextMIMC)
    #lines!(ax2,tol, slopeLineMIMC2,linewidth=2, color=:black, linestyle=:dashdot, label=slopeTextMIMC2)
    ...
    for ℓ2 = 0:Llim
        ...
        for ℓ1 =0:Llim
            if ξ1*ℓ1 + ξ2*ℓ2 ≤ L
                ...
                if reducedSample && ℓ1>0
                    if min(β1,β2) > 1
                            m[ℓ1+1, ℓ2+1] = ceil(Int, 28*tol^(-2)/max(J[ℓ1+1]^((1+β1)/2)*N[ℓ2+1]^((1+β2)/2), N[ℓ2+1]^((1+β3)/2)));

                    elseif max(β1,β2) > min(β1,β2)==1
                            m[ℓ1+1, ℓ2+1] = ceil(Int, 28*L*tol^(-2)/max(J[ℓ1+1]^((1+β1)/2)*N[ℓ2+1]^((1+β2)/2), N[ℓ2+1]^((1+β3)/2)));
                    elseif β1==β2==1
                        m[ℓ1+1, ℓ2+1] = ceil(Int, 20*L^2*tol^(-2)/max(J[ℓ1+1]^((1+β1)/2)*N[ℓ2+1]^((1+β2)/2), N[ℓ2+1]^((1+β3)/2)));
                    end
                else
                    if min(β1,β2) > 1
                        m[ℓ1+1, ℓ2+1] = ceil(Int, 28*tol^(-2)/(J[ℓ1+1]^((1+β1)/2)*N[ℓ2+1]^((1+β2)/2)));
                        
                    elseif max(β1,β2) > min(β1,β2)==1
                        m[ℓ1+1, ℓ2+1] = ceil(Int, 28*L*tol^(-2)/(J[ℓ1+1]^((1+β1)/2)*N[ℓ2+1]^((1+β2)/2)));
                    elseif β1==β2==1
                        m[ℓ1+1, ℓ2+1] = ceil(Int, 20*L^2*tol^(-2)/(J[ℓ1+1]^((1+β1)/2)*N[ℓ2+1]^((1+β2)/2)));
                    end
                end
